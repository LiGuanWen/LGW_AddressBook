...
@interface LGWSphereView() <UIGestureRecognizerDelegate>

@end

@implementation LGWSphereView
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    LGWPoint normalDirection;
    CGPoint last;
...
- (void)setCloudTags:(NSArray *)array
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        LGWPoint point = LGWPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(LGWPoint)];
    ...
    normalDirection = LGWPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(LGWPoint)direction andAngle:(CGFloat)angle
{
    ...
    LGWPoint rPoint = LGWPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(LGWPoint)];
...
- (void)setTagOfPoint: (LGWPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        LGWPoint direction = LGWPointMake(last.y - current.y, current.x - last.x, 0);
